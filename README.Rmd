...
* `printDebugHtml()` - pretty colorized HTML output, usable in RMarkdown.
* `rainbow2()` - rainbow categorical colors with enhanced visual contrast.
...
* `printDebugHtml()` - colored HTML output.
  ...
  * or enabled with `options("jam.htmlOut"=TRUE, "jam.comment"=FALSE)`
  to convert all `printDebug()` output to HTML- and Rmd-friendly format.
  Typical output includes "##" prefix, so copy-paste from console is safe,
  however in RMarkdown "##" prefix is a heading, so we add `comment=FALSE`.

* `kable_coloring()` - colorizes `kable()` and `kableExtra::kable()` output
using named vector of colors with `colorSub`.
