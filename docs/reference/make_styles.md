...
- colors:
  ...
  `integer`, default NULL, number of colors for console output, when
  NULL it calls
  [`crayon::num_colors()`](http://r-lib.github.io/crayon/reference/num_colors.md)
  to detect console capabilities.
...
- colorTransparent:
  ...
  `character` color used to substitute for "transparent" which a valid R
  color, but not a valid color for the crayon package.
...
entries are surrounded by the relevant encoding consistent with the
`style` defined at input. In short, a character vector as input, a
colorized character vector as output.
...
in order to style a vector of character strings with a vector of
